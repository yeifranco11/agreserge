...
export async function POST(request: Request) {
  try {
    ...
    const targetId = append ? randomUUID() : documentId;
    ...
    if (append) {
      const { error: insertError } = await database.from('agreserge_documents').insert({
        id: targetId, agremiado_id: userId, nombre: `${document.nombre} — ${file.name}`, categoria: document.categoria,
        archivo_path: path, archivo_nombre: file.name, archivo_tipo: file.type, archivo_tamano: file.size,
        fecha_carga: today, estado: 'Cargado', observacion: 'Documento adicional cargado, pendiente de revisión.', updated_at: new Date().toISOString(),
      });
    ...
    await database.from('agreserge_audit').insert({ usuario_id: userId, evento: `Documento cargado: ${document.nombre}`, metadata: { documentId: targetId, tipo: file.type, tamano: file.size, adicional: append } });
