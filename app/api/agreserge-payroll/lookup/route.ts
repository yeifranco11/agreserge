...
export async function POST(request: Request) {
  try {
    ...
    const result = await lookupPayrollInDrive(documento);
    ...
    return NextResponse.json({ ok: true, payroll: result.payroll, source: { spreadsheetId: '11R2hU9IzD55MBa8FivztC38boeQAxGpoMly_3yH0Ajk', tab: result.tab, updatedAt: new Date().toISOString() } });
  } catch (error: any) {
