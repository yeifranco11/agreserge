...
export function CertificateTracking() {
  ...
  const whatsapp = (person: Affiliate, certificates: Certificate[]) => {
    ...
    const detail = actionable.map(cert => `• ${cert.course}: ${cert.status === "VENCIDO" ? `VENCIDO (${formatDate(cert.expiresAt)})` : cert.status === "PROXIMO" ? `PRÓXIMO A VENCER (${formatDate(cert.expiresAt)})` : "PENDIENTE"}`).join("\n");
    const message = `Hola ${person.name}, desde AGRESERGE te informamos que debes gestionar los siguientes cursos o certificados:\n\n${detail}\n\nPor favor realiza su actualización y entrega los soportes correspondientes. Muchas gracias.`;
  ...
  return <div className="certificateModule">
    <section className="certHero"><div><span className="badge">INTELIGENCIA DOCUMENTAL</span><h2>Control de certificados</h2><p>Una ficha por afiliado, con todos sus cursos y vigencias desde la base oficial.</p></div><Award size={52}/></section>
