...
type StatusFilter = Status | "GESTION" | "";
...
const matchesStatus = (status: Status, filter: StatusFilter) => !filter || (filter === "GESTION" ? ["VENCIDO", "PROXIMO", "PENDIENTE"].includes(status) : status === filter);
...
export function CertificateTracking() {
  ...
  const [filters, setFilters] = useState<{ search: string; entity: string; area: string; course: string; status: StatusFilter }>({ search: "", entity: "", area: "", course: "", status: "GESTION" });
  const load = async () => {
  ...
  const visible = useMemo(() => (data?.affiliates || []).map(person => {
    ...
    const certificates = personMatches ? person.certificates.filter(cert =>
      (!filters.course || cert.course === filters.course) && matchesStatus(cert.status, filters.status)) : [];
  ...
  const exportWhatsappXlsx = async () => {
    ...
    const records = visible.map(({ person }) => ({ person, certificates: person.certificates.filter(isActionable) })).filter(x => x.certificates.length > 0 && x.person.phone);
  ...
  const markAlerts = async () => {
    if (!confirm("Se marcarán en la base oficial: rojo intenso los vencidos o pendientes, amarillo intenso los próximos, verde los vigentes y gris los que no aplican. ¿Continuar?")) return;
    setMarking(true);
    try {
  ...
  return <div className="certificateModule">
    ...
    <section className="certPeopleList">{visible.slice(0,1200).map(({person,certificates}) => {
      const actionable = person.certificates.filter(isActionable);
