...
import { CalendarDays, CheckCircle2, Clipboard, Download, ExternalLink, Search, Users, XCircle } from "lucide-react";
...
export function PsychotechnicalScheduling() {
  ...
  return <div className="grid">
    ...
    <div className="card span12">
      ...
      <div className="tableWrap"><table><thead><tr><th>Fecha y hora</th><th>Afiliado partícipe</th><th>Documento</th><th>Área o servicio</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{visible.map((item:any)=>{const slot:any=slotMap[item.slot_id];return <tr key={item.id}><td><b>{slot?dateLabel(slot.fecha):"—"}</b><br/><span className="muted">{slot?timeLabel(slot.hora):"—"}</span></td><td><b>{item.nombre_completo}</b></td><td>{item.documento}</td><td>{item.area}</td><td><span className="badge">{item.estado}</span></td><td><select disabled={busy} value={item.estado} onChange={(e)=>update(item.id,e.target.value)}><option>CONFIRMADA</option><option>ASISTIÓ</option><option>NO ASISTIÓ</option><option>CANCELADA</option></select></td></tr>})}{!visible.length&&<tr><td colSpan={6} className="empty">Aún no hay reservas con estos filtros.</td></tr>}</tbody></table></div>
    </div>
  </div>;
}
